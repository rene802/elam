...
model_hdl.load_model_handler("Results_Bkg_rd/model_bkg_rd.pkl")
...
#x_rd_bdt_prompt_out = np.array([y[1] for y in y_pred_rd]) # real data prompt predicted sample
#x_rd_bdt_nprompt_out = np.array([y[2] for y in y_pred_rd]) # real data nprompt predicted
x_rd_bdt_bkg_out = np.array([y[0] for y in y_pred_rd]) # real data background predicted
...
#h_rd = ndarray2th1(x_rd_bdt_nprompt_out, "h_rd", Nbins_output_score, 0, 1)
#bdt_nprompt_rd = ROOT.RooDataHist("real_data", "real_data", ROOT.RooArgList(bdt_out), h_rd)

h_rd = ndarray2th1(x_rd_bdt_bkg_out, "h_rd", Nbins_output_score, 0, 1)
bdt_bkg_rd = ROOT.RooDataHist("real_data", "real_data", ROOT.RooArgList(bdt_out), h_rd)
...
#model.fitTo(bdt_nprompt_rd, ROOT.RooFit.Extended(True))
#frame = bdt_out.frame()
#bdt_nprompt_rd.plotOn(frame)

model.fitTo(bdt_bkg_rd, ROOT.RooFit.Extended(True))
frame = bdt_out.frame()
bdt_bkg_rd.plotOn(frame)
...
#bdt_nprompt_rd.plotOn(frame, ROOT.RooFit.Name("real data"), ROOT.RooFit.MarkerStyle(20),
                        # ROOT.RooFit.MarkerSize(0.8))

bdt_bkg_rd.plotOn(frame, ROOT.RooFit.Name("real data"), ROOT.RooFit.MarkerStyle(20),
                         ROOT.RooFit.MarkerSize(0.8))
...
#file_out = ROOT.TFile("fit_prompt_bdt.root", "RECREATE")
#dir_bdt_output = file_out.mkdir("RealData - TestData")
#dir_bdt_output.cd()

#file_out1 = ROOT.TFile("fit_nprompt_bdt.root", "RECREATE")
#dir_bdt_output = file_out1.mkdir("RealData - TestData")
#dir_bdt_output.cd()

file_out2 = ROOT.TFile("fit_bkg_bdt.root", "RECREATE")
dir_bdt_output = file_out2.mkdir("RealData - TestData")
dir_bdt_output.cd()
