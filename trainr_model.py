...
# configuration
train_test_file = os.path.join(model_dir, "train_testr_data.pkl")
model_file = os.path.join(model_dir, "model_r.pkl")
study_file = os.path.join(model_dir, "optuna_studyr.pkl")
...
variables_to_exclude = ['fMass', 'fQtAP','fCt','fRadius', 'fTpcNsigmaPos', 'fTpcNsigmaNeg','fPt','fRadius','fEta'] #, 'fTpcNsigmaPos', 'fTpcNsigmaNeg', 'fPt',,'fRadius','fEta']
...
# keep variables contributing to 97% of the importance
...
print(f"After importance (97%)  : {len(features_selection)}")
...
#########################################################################################################################
# plotting of the feature importance with SHAP values, to verify which variables 
# are important for the classification task, 
###########################################################################################################################
def plot_feature_imp(train_test_data, model_hdl, labels, n_sample=100000, approximate=True):
...
res = plot_feature_imp(train_test_data, model_hdl, leg_labels, n_sample=100000, approximate=True)
...
for i in range(len(ml_out_fig)):    
    ...
    latex.DrawLatex(0.15, 0.83, "1 < #it{p}_{T} < 5 GeV/#it{c}")
