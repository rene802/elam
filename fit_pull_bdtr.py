...
# configuration
parquet_file = "rd.parquet.gzip"
test_parquet = "rd_test.parquet.gzip"
output_root = "fit_bdt_prompt_pull.root"
...
# save image
c_pull_dist.SaveAs("pull_distribution.png")
...
#test_prompt_nonpromptRegion = df_test[
#    (df_test["label"] == 1) &
#    (df_test["model_output_2"] > 0.035) &
#    (df_test["model_output_2"] < 0.115)]
...
#test_nprompt_promptRegion = df_test[
#    (df_test["label"] == 2) &
#    (df_test["model_output_2"] > 0.9)]
...
outfile_features = ROOT.TFile("features_after_bdt_cut.root","RECREATE")
...
for region_name_p, (rd_sel_p, prompt_sel) in regions_p.items():
    for var in vars_to_plot:
        ...
        c_prompt.SaveAs(f"{var}_{region_name_p}_prompt.png")
        outfile_features.cd()

         #h_rd_prompt = h_rd.Clone(f"h_rd_prompt_{var}_{region_name}")
         #h_prompt_clone = h_prompt.Clone(f"h_prompt_{var}_{region_name}")
        h_rd_p.Write()
        h_prompt.Write()
        c_prompt.Write()
        c_prompt.Close()
...
for region_name_np, (rd_sel_np, nprompt_sel) in regions_np.items():            
    for var in vars_to_plot:                
        ...
        # Canvas nonprompt
        # ==========================================
        c_nprompt = ROOT.TCanvas(f"c_nprompt_{var}_{region_name_np}",f"{var} nonprompt",900,700)
        ...
        c_nprompt.SaveAs(f"{var}_{region_name_np}_nonprompt.png")
...
# Output ROOT file
################################################
outfile = ROOT.TFile("purity_scan.root","RECREATE")
...
c_prompt.Write()
c_prompt.SaveAs("prompt_optimization.png")
...
c_nprompt.Write()
c_nprompt.SaveAs("nonprompt_optimization.png")
