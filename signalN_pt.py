...
from configN_analysis import *
ROOT.gInterpreter.ProcessLine('#include "RooDSCBShape.h"')
ROOT.gInterpreter.ProcessLine('.L RooDSCBShape.cxx')
#print(ROOT.RooDSCBShape)


def compute_chi2_th1(h_data, h_model, npar):
    chi2 = 0.0
    nused = 0
    for i in range(1, h_data.GetNbinsX() + 1):
        obs = h_data.GetBinContent(i)
        exp = h_model.GetBinContent(i)
        # Pearson chi2 for Poisson counts
        if exp <= 0:
            continue
        chi2 += (obs - exp)**2 / exp
        nused += 1
    ndf = max(nused - npar, 1)
    chi2_ndf = chi2 / ndf
...
GenLambda = MC_Data.get_subset("1 < fPt < 4 and fPDGCode == 3122 and (abs(fPDGCodeMother) != 3312 and abs(fPDGCodeMother) != 3322 and abs(fPDGCodeMother) != 3334)")
...
# imput parquet
test_parquet = os.path.join(model_dir,"data_applied_sm_pt.parquet.gzip")
print("\nLoading parquet...\n")
...
real_parquet = os.path.join(model_dir,"data.parquet.gzip")
print("\nLoading parquet...\n")
...
for ptmin, ptmax in pt_bins:
    ...
    # Selection
    df_bin = df[(df["Pt"] >= ptmin) & (df["Pt"] < ptmax) & (df["model_output_1"] > bdt_cut)]
    masses = df_bin["Mass"].to_numpy()
    ...
    alphaL = ROOT.RooRealVar(f"alphaL_{ptmin}_{ptmax}","alphaL",1.5,0.,5.)
    alphaR = ROOT.RooRealVar(f"alphaR_{ptmin}_{ptmax}","alphaR",2.0,0.,5.)

    nL = ROOT.RooRealVar(f"nL_{ptmin}_{ptmax}","nL",5.,1.,30.)
    nR = ROOT.RooRealVar(f"nR_{ptmin}_{ptmax}","nR",5.,1.,30.)

    signal_pdf = ROOT.RooDSCBShape(f"signal_pdf_{ptmin}_{ptmax}","",mass_mc,mean,sigma,alphaL,nL,alphaR,nR)


    # BACKGROUND PDF (Exponential)
    tau = ROOT.RooRealVar(f"tau_{ptmin}_{ptmax}","tau",-30,-100,100)
    ...
    if efficiency > 0:
        efficiency_error = signal_yield_err / N_gen
    else:
        efficiency_error = 0
    ...
    df_bin_data = df_data[(df_data["Pt"] >= ptmin) & (df_data["Pt"] < ptmax) & (df_data["model_output_1"] > bdt_cut)]
    mass_raw = df_bin_data["Mass"].to_numpy()
    ...
    mass = ROOT.RooRealVar(f"mass_{ptmin}_{ptmax}","Mass(GeV/c^{2})",mass_range[0],mass_range[1])
    ...
    alphaL_raw = ROOT.RooRealVar(f"alphaL_raw_{ptmin}_{ptmax}","",alphaL.getVal(),0.,5)
    nL_raw = ROOT.RooRealVar(f"nL_raw_{ptmin}_{ptmax}","",nL.getVal(),1,30)

    alphaR_raw = ROOT.RooRealVar(f"alphaR_raw_{ptmin}_{ptmax}","",alphaR.getVal(),0.,5)
    nR_raw = ROOT.RooRealVar(f"nR_raw_{ptmin}_{ptmax}","",nR.getVal(), 1,30)

    signal_pdf_raw = ROOT.RooDSCBShape(f"signal_pdf_raw_{ptmin}_{ptmax}","",mass,mean_raw,sigma_raw,alphaL_raw,nL_raw,alphaR_raw,nR_raw)
    ...
    # Background PDF (Exponential)
    tau_raw = ROOT.RooRealVar(f"tau_raw_{ptmin}_{ptmax}","tau",-30,-100,100)
    ...
    # Histogram of the fitted model
    h_model_raw = model_raw.createHistogram(f"h_model_raw_{ptmin}_{ptmax}",mass,ROOT.RooFit.Binning(Nbins, mass_range[0], mass_range[1]))

    # Normalize model histogram to the number of data entries
    model_integral = h_model_raw.Integral()
    data_integral = h_mass_raw.Integral()
    if model_integral > 0:
        h_model_raw.Scale(data_integral / model_integral)
        # Number of floating parameters
        npar = fit_result_raw.floatParsFinal().getSize()
        # Pearson chi2
        chi2, ndf, chi2_ndf = compute_chi2_th1(h_mass_raw,h_model_raw,npar)
        print("----- GOODNESS OF FIT -----")
        print(f"Number of floating parameters = {npar}")
    ...
    if N_raw_data > 0 and efficiency > 0:        
        yield_density = (N_raw_data /delta_pt /efficiency/Nev/br)
    ...
    c = ROOT.TCanvas(f"c_raw_{ptmin}_{ptmax}","",1000,600)

    frame.Draw()
    latex = ROOT.TLatex()
    latex.SetNDC()
    latex.SetTextSize(0.04)
    latex.DrawLatex(0.18,0.80, "pp #sqrt{s} = 13.6 TeV")
    ...
    #latex.DrawLatex(0.58,0.60, f"#chi^{{2}} = {chi2:.1f}")
    #latex.DrawLatex(0.58,0.54, f"ndf = {ndf}")
    latex.DrawLatex(0.58,0.60,f"#chi^{{2}}/ndf = {chi2_ndf:.2f}")
...
# ============================================================
# FINAL EFFICIENCY vs pT
# ============================================================

eff_x = np.array([0.5 * (ptmin + ptmax) for ptmin, ptmax in pt_bins],dtype=np.float64)
eff_y = np.asarray(eff_vals, dtype=np.float64)
eff_yerr = np.asarray(eff_errs, dtype=np.float64)
eff_xerr = np.asarray([0.5 * (ptmax - ptmin) for ptmin, ptmax in pt_bins],dtype=np.float64)

# Make arrays contiguous for PyROOT
eff_x = np.ascontiguousarray(eff_x)
eff_y = np.ascontiguousarray(eff_y)
eff_xerr = np.ascontiguousarray(eff_xerr)
eff_yerr = np.ascontiguousarray(eff_yerr)

print("\n===== EFFICIENCY GRAPH =====")
print("Number of points:", len(eff_x))
print("x:", eff_x)
print("y:", eff_y)
print("x errors:", eff_xerr)
print("y errors:", eff_yerr)

eff_graph = ROOT.TGraphErrors(len(eff_x),eff_x,eff_y,eff_xerr,eff_yerr)
c_eff = ROOT.TCanvas("c_eff", "", 900, 700)
c_eff.SetLeftMargin(0.15)
c_eff.SetBottomMargin(0.13)
c_eff.SetTopMargin(0.08)
c_eff.SetRightMargin(0.05)

eff_graph.SetTitle("Efficiency Vs #it{p}_{T} (GeV/c)")

eff_graph.SetMarkerStyle(20)
eff_graph.SetMarkerSize(1.3)
...
eff_graph.GetYaxis().SetTitleSize(0.045)
eff_graph.GetYaxis().SetTitleOffset(1.3)
eff_graph.GetYaxis().SetLabelSize(0.04)

# X-axis
eff_graph.GetXaxis().SetTitle("p_{T} (GeV/c)")
eff_graph.GetXaxis().SetTitleSize(0.045)
eff_graph.GetXaxis().SetTitleOffset(1.1)
eff_graph.GetXaxis().SetLabelSize(0.04)

eff_graph.Draw("AP")

latex_eff = ROOT.TLatex()
latex_eff.SetNDC()
latex_eff.SetTextSize(0.040)

latex_eff.DrawLatex(0.18, 0.40,"pp #sqrt{s} = 13.6 TeV")
latex_eff.DrawLatex(0.18, 0.45,"#it{This Thesis}")
latex_eff.DrawLatex(0.18, 0.35,"1 < #it{p}_{T} < 4 GeV/#it{c}")
latex_eff.DrawLatex(0.18, 0.30,"1 < #it{ct} < 30 cm")

out.cd()

eff_graph.Write("efficiency_vs_pt")
c_eff.Write("c_eff")

eff_plot_path = os.path.join(plot_dir,"efficiency_vs_pt.png")
c_eff.SaveAs(eff_plot_path)

print(f"Efficiency plot saved in: {eff_plot_path}")
...
graph.SetTitle(";#it{p}_{T} (GeV/c);dN_{corr}/dp_{T} (GeV/c)^{-1}")
...
latex.DrawLatex(0.18,0.45,"This Thesis")
latex.DrawLatex(0.18,0.40, "pp #sqrt{s} = 13.6 TeV")
latex.DrawLatex(0.18,0.35, "1 < #it{p}_{T} < 4 GeV/#it{c}")
latex.DrawLatex(0.18,0.30, "1 < #it{ct} < 30 cm")
...
# Colomun
table_data = [["pT min","pT max","pT center","Ngen","Nreco","Efficiency","Err(Eff)","1/Eff","Err(1/Eff)","(1/ε) dNraw/dpT/Nev","(1/ε) dNraw/dpT_error"]]
